...
const works = [
  { image: "/images/pulido1.webp",   title: "Pulido Profesional",  number: "01" },
  { image: "/images/interior.webp",  title: "Interior Detallado",  number: "02" },
  { image: "/images/exterior.webp",  title: "Detailing Exterior",  number: "03" },
  { image: "/images/ceramica1.webp", title: "Protección Cerámica", number: "05" },
];

export default function Works() {
  const [activeIndex, setActiveIndex] = useState(0);

  const nextSlide = () => setActiveIndex((prev) => (prev === works.length - 1 ? 0 : prev + 1));
  const prevSlide = () => setActiveIndex((prev) => (prev === 0 ? works.length - 1 : prev - 1));

  // Fix: sin [activeIndex] como dependencia + updater fn para evitar stale closure
  useEffect(() => {
  ...
  const getPrevIndex = () => (activeIndex === 0 ? works.length - 1 : activeIndex - 1);
  const getNextIndex = () => (activeIndex === works.length - 1 ? 0 : activeIndex + 1);

  return (
    <section id="works" className="relative overflow-hidden bg-black py-24">
      ...
      <div className="relative z-10 mx-auto max-w-[1700px]">
        ...
        >
          ...
          <h2 className="mt-4 font-[family-name:var(--font-bebas)] text-5xl uppercase leading-none text-white md:text-7xl">
            ...
            <span className="block">Por Sí Solos</span>
          </h2>
